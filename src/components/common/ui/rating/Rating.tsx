...
export function Rating({rate, hideText}: Props) {
    ...
    for (let i = 0; i < Math.trunc(rate); i++) {
        stars.push(<li className="flex" key={i}><IconBox icon={"icon-star"} iconClassName={"text-yellow"} size={12}/></li>)
    }
    let emptyStars = []
    for (let i = Math.trunc(rate); i < (Number.isInteger(rate) ? 5 : 4); i++) {
        emptyStars.push(<li className="flex" key={i}><IconBox icon={"icon-star-o"} iconClassName={"text-yellow"} size={12}/></li>)
    }
    return (
        <>
